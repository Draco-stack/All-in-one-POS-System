...
export const AdminReportsAnalytics: React.FC = () => {
  ...
  const handleExportCSV = () => {
    if (selectedReportTab === 'SHIFTS') {
      ...
      link.href = url;
      link.download = `Shifts_Report_${new Date().toISOString().slice(0, 10)}.csv`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      showToast('✓ Shift audit ledger downloaded as CSV');
  ...
  return (
    <div className="space-y-6">
      {/* Top Tabs */}
      <div className="bg-stone-900 border border-stone-800 rounded-2xl p-4.5 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-[#00897b] hover:bg-[#00796b] text-white text-xs font-bold flex items-center gap-1.5 transition cursor-pointer"
          >
        ...
        <div className="bg-stone-900 border border-stone-800 rounded-2xl p-5 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-stone-800">
            ...
            <input
              ...
              className="px-3 py-1.5 bg-stone-950 border border-stone-800 rounded-xl text-xs text-white placeholder-stone-500 w-full sm:w-60 focus:outline-none focus:border-[#00897b]"
            />
          </div>
            ...
            <table className="w-full text-left text-xs text-stone-300 border-collapse">
              ...
              <tbody className="divide-y divide-stone-800/60 font-sans">
                {filteredShifts.map((sh) => {
                  ...
                  return (
                    <tr key={sh.id} className="hover:bg-stone-800/40 transition">
      ...
      {selectedReportTab === 'MONTHLY' && (
        <div className="bg-stone-900 border border-stone-800 rounded-2xl p-5 shadow-sm space-y-4">
          <div className="pb-3 border-b border-stone-800 flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-[#00897b] font-bold">Base Currency: PKR</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {monthlySummaries.map((m, idx) => (
              <div key={idx} className="p-4 bg-stone-950 rounded-xl border border-stone-800/80 space-y-3">
                ...
                <div className="space-y-1 text-xs border-y border-stone-800/60 py-2 font-mono">
                ...
                {topSellingItems.map((it, idx) => (
                  <tr key={idx} className="hover:bg-stone-800/40 transition">
                    ...
                    <td className="py-3 px-3">
                      <span className="px-2 py-0.5 rounded bg-stone-800 text-stone-300 text-[10px] uppercase font-mono">
                        {it.category}
                      </span>
                    </td>
                    <td className="py-3 px-3 text-right font-mono font-semibold text-white">

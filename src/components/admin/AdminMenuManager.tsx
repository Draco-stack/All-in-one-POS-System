    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all duration-150 cursor-pointer border active:scale-95 shadow-sm ${
                theme === 'dark' 
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white text-xs font-bold flex items-center gap-1.5 shadow-sm active:scale-95 transition-all duration-150 cursor-pointer border border-emerald-500/30"
            >
        ...
        <div className={`flex flex-col sm:flex-row items-center gap-3 pt-3 border-t ${theme === 'dark' ? 'border-white/5' : 'border-slate-200'}`}>
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className={`w-full pl-9 pr-3 py-1.5 rounded-xl text-xs transition border focus:outline-none focus:border-emerald-500 ${
                theme === 'dark' 
          ...
          <table className={`w-full text-left text-xs border-collapse ${theme === 'dark' ? 'text-stone-300' : 'text-slate-700'}`}>
            <thead>
              <tr className={`border-b font-bold uppercase tracking-wider text-[10px] ${theme === 'dark' ? 'border-white/10 text-stone-400 bg-[#0c0d12]' : 'border-slate-200 text-slate-500 bg-slate-50'}`}>
                ...
                <th className="py-3 px-3 text-right">Floor Price (PKR)</th>
                <th className="py-3 px-3">Custom Flavors</th>
                <th className="py-3 px-3 text-center">POS Status</th>
                  ...
                  return (
                    <tr
                      ...
                      className={`transition ${
                        !isAvail 
                          ? (theme === 'dark' ? 'opacity-55 bg-black/20' : 'opacity-65 bg-slate-50/80') 
                    ...
                    >
                      ...
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <div>
                            <div className={`font-bold flex items-center gap-1.5 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                              {item.name}
                              {item.isPopular && (
                           ...
                           <div className="inline-flex items-center gap-1">
                            <input
                              ...
                              className={`w-20 px-2 py-1 border border-emerald-500 rounded text-xs font-mono text-right focus:outline-none ${
                                theme === 'dark' ? 'bg-[#08090d] text-white' : 'bg-slate-50 text-slate-800'
                              }`}
                              autoFocus
                              onKeyDown={(e) => {
                            ...
                            <button
                              ...
                              className="p-1 text-stone-400 hover:text-stone-300 cursor-pointer"
                            >
                      ...
                      <td className="py-3 px-3">
                        <div className="flex flex-wrap gap-1 max-w-xs">
                          {item.flavors && item.flavors.length > 0 ? (
                            item.flavors.map((f, i) => (
                              <span
                                ...
                                className={`px-1.5 py-0.2 rounded text-[10px] border ${
                                  theme === 'dark' 
                                    ? 'bg-[#08090d] text-stone-400 border-white/5' 
                                    : 'bg-slate-100 text-slate-600 border-slate-200'
                                }`}
                              >
                                {f}
                            ...
                            <span className="text-stone-400 italic text-[11px]">Standard</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-3 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className={`p-1.5 rounded-lg transition-all duration-150 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed border active:scale-95 ${
                              theme === 'dark'
                                ? 'text-stone-400 hover:text-rose-400 bg-stone-800/80 hover:bg-rose-950/40 border-white/5'
                                : 'text-slate-600 hover:text-rose-600 bg-slate-100 hover:bg-rose-50 border-slate-200'
                            }`}
                            title="Delete Item"
                          >
      ...
      {isItemModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className={`border rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 ${
            theme === 'dark' ? 'bg-[#12141c] border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900 shadow-2xl'
          }`}>
            ...
            <form onSubmit={handleSaveItem} className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
              {/* Item Title */}
              <div>
                <label className={`block text-xs font-semibold mb-1 ${theme === 'dark' ? 'text-stone-300' : 'text-slate-700'}`}>Item Title / Name *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={`block text-xs font-semibold mb-1 ${theme === 'dark' ? 'text-stone-300' : 'text-slate-700'}`}>Price (PKR) *</label>
                  <input
                    type="number"
                    required
                    min="1"
                ...
                <div>
                  <label className={`block text-xs font-semibold mb-1 ${theme === 'dark' ? 'text-stone-300' : 'text-slate-700'}`}>Category *</label>
                  <select
                    value={itemFormData.category}
                    onChange={(e) => setItemFormData({ ...itemFormData, category: e.target.value })}
                    className={`w-full px-3 py-2.5 rounded-xl text-xs focus:outline-none focus:border-emerald-500 transition cursor-pointer border ${
                      theme === 'dark'
                        ? 'bg-[#08090d] border-white/10 text-white'
                        : 'bg-slate-50 border-slate-300 text-slate-900'
                    }`}
                  >
                    {categories.map((c) => (
                      <option key={c.id} value={c.id} className={theme === 'dark' ? 'bg-[#08090d] text-white' : 'bg-white text-slate-900'}>
                        {c.name}
                      </option>
                    ))}
                  </select>
              ...
              {/* Description */}
              <div>
                <label className={`block text-xs font-semibold mb-1 ${theme === 'dark' ? 'text-stone-300' : 'text-slate-700'}`}>Description & Ingredients</label>
                <textarea
                  rows={2}
                  placeholder="e.g. Signature marinated chicken cubes with diced onions, bell peppers and mozzarella..."
                  value={itemFormData.description}
                  onChange={(e) => setItemFormData({ ...itemFormData, description: e.target.value })}
                  className={`w-full px-3 py-2 rounded-xl text-xs focus:outline-none focus:border-emerald-500 transition border ${
                    theme === 'dark'
                      ? 'bg-[#08090d] border-white/10 text-white placeholder-stone-500'
                      : 'bg-slate-50 border-slate-300 text-slate-900 placeholder-slate-400'
                  }`}
              ...
              <div>
                <label className={`block text-xs font-semibold mb-1 ${theme === 'dark' ? 'text-stone-300' : 'text-slate-700'}`}>Flavor / Style Options (Comma separated)</label>
                <input
                  type="text"
              ...
              <div className={`flex items-center justify-between pt-2 border-t ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
                <label className={`flex items-center gap-2 text-xs font-semibold cursor-pointer ${theme === 'dark' ? 'text-stone-300' : 'text-slate-700'}`}>
                  <input
                    type="checkbox"
              ...
              <div className={`flex items-center justify-end gap-2 pt-4 border-t ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
                <button
                  type="button"
                  onClick={() => setIsItemModalOpen(false)}
                  className={`px-4 py-2 rounded-xl text-xs font-semibold transition cursor-pointer border active:scale-95 ${
                    theme === 'dark'
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white text-xs font-bold transition-all duration-150 cursor-pointer shadow-sm active:scale-95 border border-emerald-500/30"
                >
          ...
          <div className={`border rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 ${
            ...
            <div className={`p-4.5 border-b flex items-center justify-between ${
              ...
              <div className="flex items-center gap-2">
                ...
                <h4 className={`text-sm font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Dynamic Category Control</h4>
              </div>
              <button
            ...
            <div className="p-5 space-y-4">
              ...
              <form onSubmit={handleAddCategorySubmit} className="flex gap-2">
                <input
                  ...
                  className={`flex-1 px-3 py-2.5 rounded-xl text-xs focus:outline-none focus:border-emerald-500 transition border ${
                    theme === 'dark'
                      ? 'bg-[#08090d] border-white/10 text-white placeholder-stone-500'
                      : 'bg-slate-50 border-slate-300 text-slate-900 placeholder-slate-400'
                  }`}
                ...
                <button
                  type="submit"
                  className="px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white text-xs font-bold rounded-xl transition-all duration-150 cursor-pointer shadow-sm active:scale-95 border border-emerald-500/30"
                >
                  Add
                </button>
              </form>
                ...
                {categories.map((c, index) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-2.5 rounded-xl border transition ${
                      theme === 'dark'
                        ? 'bg-[#08090d] border-white/5 hover:border-white/10'
                    ...
                    {editingCatId === c.id ? (
                      <div className="flex items-center gap-1.5 flex-1">
                        <input
                          ...
                          className={`flex-1 px-2 py-1 border border-emerald-500 rounded text-xs ${
                            theme === 'dark' ? 'bg-stone-900 text-white' : 'bg-white text-slate-900'
                          }`}
                          autoFocus
                        />
                        <button
                        ...
                        <button onClick={() => setEditingCatId(null)} className="p-1 text-stone-400 hover:text-stone-300 cursor-pointer">
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    ) : (
                      <>
                        <div className="flex items-center gap-2">
                          <span className="text-stone-400 font-mono text-[10px] w-4">{index + 1}.</span>
                          <span className={`font-semibold text-xs ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{c.name}</span>
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className={`p-1 disabled:opacity-30 cursor-pointer transition ${
                              theme === 'dark' ? 'text-stone-400 hover:text-white' : 'text-slate-500 hover:text-slate-900'
                            }`}
                            title="Move Up"
                          >
                          ...
                          <button
                            ...
                            className="p-1 text-stone-400 hover:text-rose-400 cursor-pointer transition"
                            title="Delete"
                          >
              ...
              <div className={`pt-3 border-t text-right ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
                <button
                  onClick={() => setIsCatModalOpen(false)}
                  className={`px-4 py-2 rounded-xl text-xs font-semibold transition cursor-pointer border active:scale-95 ${
                    theme === 'dark'

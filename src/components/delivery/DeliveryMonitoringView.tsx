...
export const DeliveryMonitoringView: React.FC<DeliveryMonitoringViewProps> = ({
  ...
  // Helper for Order Status Badge with Status Light Reflections
  const getOrderStatusBadge = (order: Order) => {
    const st = (order.status || 'pending').toLowerCase();
    
    // 1. Delivered / Completed -> Reflects Green Light
    if (st === 'delivered' || st === 'completed') {
      return (
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-emerald-500/25 text-emerald-300 border border-emerald-400/60 text-xs font-black tracking-wide uppercase shadow-[0_0_12px_rgba(52,211,153,0.45)] ring-1 ring-emerald-400/30">
          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 animate-pulse" />
          <span>Delivered</span>
        </div>
      );
    }
    // 2. On The Way / Dispatched -> Reflects Yellow-Orange Light
    if (st === 'dispatched' || st === 'on_the_way' || st === 'in_transit') {
      return (
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-amber-500/25 text-amber-300 border border-amber-400/60 text-xs font-black tracking-wide uppercase shadow-[0_0_14px_rgba(245,158,11,0.45)] ring-1 ring-orange-400/30">
          <Truck className="w-3.5 h-3.5 text-amber-400 animate-bounce" />
          <span>On The Way</span>
        </div>
      );
    }
    // 3. In Kitchen / Preparation
    if (st === 'in_kitchen' || st === 'pending' || st === 'open' || st === 'punched') {
      return (
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-indigo-500/20 text-indigo-300 border border-indigo-400/40 text-xs font-bold tracking-wide uppercase shadow-[0_0_10px_rgba(99,102,241,0.25)]">
    ...
    if (st === 'ready') {
      return (
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-400/50 text-xs font-black tracking-wide uppercase shadow-[0_0_12px_rgba(52,211,153,0.35)]">
          <Sparkles className="w-3.5 h-3.5 text-emerald-400 animate-pulse" />
          <span>Ready</span>
        </div>
      );
    }
    // 5. Cancelled / Refunded / Void -> Reflects Red Light
    return (
      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-rose-500/25 text-rose-300 border border-rose-500/60 text-xs font-black tracking-wide uppercase shadow-[0_0_14px_rgba(244,63,94,0.45)] ring-1 ring-rose-500/30">
        <Ban className="w-3.5 h-3.5 text-rose-400" />
        <span>{st === 'refunded' ? 'Refunded' : 'Cancelled'}</span>
      </div>
    );
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {Object.entries(clusteredSectors).map(([sectorName, sectorOrders]) => {
                  ...
                  return (
                    <div
                      key={sectorName}
                      className={`relative overflow-hidden border-2 rounded-2xl p-5 flex flex-col justify-between space-y-4 shadow-lg transition-all duration-200 ${
                        theme === 'dark'
                          ? 'bg-gradient-to-b from-[#151722] via-[#10121a] to-[#0c0d12] border-blue-400/60 shadow-[0_0_20px_rgba(59,130,246,0.18)] hover:border-blue-400/90 hover:shadow-[0_0_25px_rgba(59,130,246,0.28)]'
                          : 'bg-white border-slate-200 shadow-sm hover:border-slate-300'
                      }`}
                    >
                      {/* Top Ambient Glow Strip */}
                      {theme === 'dark' && (
                        <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 via-indigo-300 to-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.8)]" />
                      )}
                      ...
                      <div>
                        <div className={`flex items-center justify-between border-b pb-3 ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
                          ...
                          <span className="px-2.5 py-0.5 rounded-full bg-blue-500/20 border border-blue-400/40 text-blue-400 text-xs font-mono font-bold shadow-xs">
                ...
                <tbody className={`divide-y text-xs font-medium ${theme === 'dark' ? 'divide-white/5' : 'divide-slate-200'}`}>
                  {displayedOrders.length === 0 ? (
                    ...
                    displayedOrders.map((order) => {
                      ...
                      const st = (order.status || 'pending').toLowerCase();
                      const isCancelled = st === 'cancelled' || st === 'refunded' || st === 'void';
                      const isDelivered = st === 'delivered' || st === 'completed';
                      const isOnTheWay = st === 'dispatched' || st === 'on_the_way' || st === 'in_transit';

                      const rowLightClass = isCancelled
                        ? theme === 'dark' 
                          ? 'bg-red-950/15 border-l-4 border-l-rose-500 shadow-[inset_0_0_12px_rgba(244,63,94,0.08)]' 
                          : 'bg-red-50/50 border-l-4 border-l-rose-500'
                        : isDelivered
                        ? theme === 'dark'
                          ? 'bg-emerald-950/15 border-l-4 border-l-emerald-500 shadow-[inset_0_0_12px_rgba(52,211,153,0.08)]'
                          : 'bg-emerald-50/50 border-l-4 border-l-emerald-500'
                        : isOnTheWay
                        ? theme === 'dark'
                          ? 'bg-amber-950/15 border-l-4 border-l-amber-400 shadow-[inset_0_0_12px_rgba(245,158,11,0.08)]'
                      ...
                      return (
                        <tr
                          key={order.id}
                          className={`transition-colors group ${rowLightClass} ${
                            theme === 'dark' ? 'hover:bg-white/[0.04] text-stone-200' : 'hover:bg-slate-50 text-slate-800'
                          }`}
                        >
                          {/* Order ID & Source */}

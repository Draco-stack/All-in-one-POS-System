...
export const ParkedOrdersModal: React.FC<ParkedOrdersModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 dark:bg-black/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-150">
      <div className="bg-gradient-to-b from-stone-900 to-[#121212] border border-slate-300 dark:border-white/10 ring-1 ring-white/10 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="p-4 bg-white/60 dark:bg-stone-950/60 backdrop-blur-xs border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase font-bold tracking-wider text-slate-500 dark:text-stone-400">
        ...
        <div className="p-5 overflow-y-auto space-y-3 flex-1">
          {parkedOrders.length === 0 ? (
            <div className="text-center py-12 text-slate-400 dark:text-stone-500">
              ...
              <p className="text-sm font-bold text-slate-500 dark:text-stone-400">No orders currently parked</p>
            ...
            parkedOrders.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="bg-stone-950/80 border border-slate-200 dark:border-white/5 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-amber-500/30 transition-all shadow-inner"
                >
                  <div className="space-y-1.5 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black text-slate-700 dark:text-stone-300 uppercase tracking-wider">
                        {(p.cart?.orderType || (p as any).orderType || 'takeaway').replace('_', ' ')}
                      </span>
                      <span className="text-[10px] text-slate-500 dark:text-stone-400 font-bold flex items-center gap-1">
                        <Clock className="w-3 h-3 text-amber-400" />
                        {timeFormatted}
                      </span>
                    </div>

                    <div className="flex items-center gap-3 text-xs text-slate-700 dark:text-stone-300">
                      <span className="flex items-center gap-1 font-bold">
                        <User className="w-3.5 h-3.5 text-slate-500 dark:text-stone-400" />
                        {p.cart?.customer?.name || (p as any).customerName || p.title || 'Guest'}
                      </span>
                      {(p.cart?.customer?.phone || (p as any).customerPhone) && (
                        <span className="flex items-center gap-1 text-slate-500 dark:text-stone-400 font-mono">
                    ...
                    <p className="text-xs text-slate-500 dark:text-stone-400 truncate max-w-md">
                  ...
                  <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 pt-2 sm:pt-0 border-slate-200 dark:border-white/5">
                    <div className="text-right">
                      <span className="text-[9px] text-slate-500 dark:text-stone-400 font-bold uppercase tracking-wider block">Items ({(p.cart?.items || (p as any).items || []).length})</span>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-2 rounded-xl bg-white dark:bg-stone-900 border border-slate-200 dark:border-white/5 hover:bg-rose-950/40 text-slate-500 dark:text-stone-400 hover:text-rose-400 transition-all cursor-pointer"
        ...
        {/* Footer */}
        <div className="p-3.5 bg-white/60 dark:bg-stone-950/60 border-t border-slate-200 dark:border-white/5 flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-white dark:bg-stone-900 hover:bg-slate-100 dark:hover:bg-stone-800 text-slate-700 dark:text-stone-300 border border-slate-200 dark:border-white/5 text-xs font-bold transition-all cursor-pointer"
          >
            Close
          </button>
        </div>

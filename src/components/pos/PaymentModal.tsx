...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 dark:bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-gradient-to-b from-stone-900 to-[#121212] border border-slate-300 dark:border-white/10 ring-1 ring-white/10 rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150 flex flex-col">
        {/* Header */}
        <div className="p-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between bg-white/60 dark:bg-stone-950/60 backdrop-blur-xs">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase font-bold tracking-wider text-slate-500 dark:text-stone-400">Select payment method & tender amount</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-4 bg-stone-950/90 rounded-2xl border border-slate-200 dark:border-white/5 shadow-inner">
              <span className="text-[10px] uppercase font-bold tracking-wider text-slate-500 dark:text-stone-400">Total Amount Due</span>
            ...
            <div className={`p-4 rounded-2xl border ${changeDue >= 0 ? 'bg-emerald-950/40 border-emerald-500/30 shadow-inner' : 'bg-red-950/40 border-red-500/30 shadow-inner'}`}>
              <span className="text-[10px] uppercase font-bold tracking-wider text-slate-500 dark:text-stone-400">Change Due to Customer</span>
          ...
          {paymentMethod === 'cash' && (
            <div className="space-y-3">
              <div className="flex gap-2">
                ...
                {quickCashBills.map((bill) => (
                  <button
                    ...
                    className="flex-1 py-2 rounded-xl bg-stone-950/90 hover:bg-slate-100 dark:hover:bg-stone-800 text-xs font-bold font-mono text-slate-700 dark:text-stone-300 border border-slate-200 dark:border-white/5 transition-all cursor-pointer"
                  >
                    PKR {bill}
                  </button>
                ))}
              ...
              <div className="bg-stone-950/90 p-3 rounded-2xl border border-slate-200 dark:border-white/5 shadow-inner">
                <div className="text-right px-4 py-2.5 bg-white dark:bg-stone-900/90 rounded-xl text-2xl font-black font-mono text-emerald-400 tracking-wider mb-2.5 border border-slate-200 dark:border-white/5 shadow-inner">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['1', '2', '3', '4', '5', '6', '7', '8', '9', 'C', '0', 'BS'].map((k) => (
                    <button
                      ...
                      className="py-3 rounded-xl bg-white dark:bg-stone-900 hover:bg-slate-100 dark:hover:bg-stone-800 text-stone-200 font-bold text-sm font-mono border border-slate-200 dark:border-white/5 transition-all active:scale-95 cursor-pointer shadow-sm"
                    >
                      {k === 'BS' ? '⌫' : k}
                    </button>
                  ))}
        ...
        <div className="p-4 border-t border-slate-200 dark:border-white/5 bg-white/60 dark:bg-stone-950/60 flex items-center justify-between">
          <button
            onClick={onClose}
            className="px-4 py-2.5 rounded-xl bg-white dark:bg-stone-900 hover:bg-slate-100 dark:hover:bg-stone-800 text-slate-700 dark:text-stone-300 font-bold text-xs border border-slate-200 dark:border-white/5 transition-all cursor-pointer"
          >
            Cancel
          </button>
          <button

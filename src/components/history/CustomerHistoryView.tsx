...
export const CustomerHistoryView: React.FC<CustomerHistoryViewProps> = ({ onClose, initialPhone = '' }) => {
  ...
  return (
    <div className="w-full h-full flex overflow-hidden bg-[#0c0c0e] text-[#e4e4e7] font-['Inter',sans-serif] select-none absolute inset-0 z-50">
      {/* Sidebar Navigation */}
      <aside className="w-[80px] bg-[#141417] border-r border-[#e4e4e7]/10 flex flex-col items-center py-6 gap-8 shrink-0 z-20">
        ...
        <div className="w-12 h-12 rounded-xl flex items-center justify-center text-[#e4e4e7]/60 hover:bg-[#e4e4e7]/10 hover:text-[#e4e4e7] cursor-pointer transition-all border border-transparent">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
        </div>
        <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white bg-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.3)] cursor-pointer transition-all border border-transparent">
        ...
        <div className="mt-auto w-12 h-12 rounded-xl flex items-center justify-center text-[#e4e4e7]/60 hover:bg-[#e4e4e7]/10 hover:text-[#e4e4e7] cursor-pointer transition-all border border-transparent" onClick={onClose}>
      ...
      <main className="flex-1 flex flex-col h-full bg-[radial-gradient(circle_at_top_right,#1a1a20,#0c0c0e)] relative overflow-hidden">
        ...
        {/* Top Filters Header */}
        <div className="px-8 py-4 bg-[#141417]/50 backdrop-blur-md flex items-center gap-4 shrink-0 border-b border-[#e4e4e7]/10 z-20">
          <select className="bg-transparent text-[#e4e4e7] border border-[#e4e4e7]/10 rounded-md px-3 py-2 text-[0.85rem] focus:outline-none appearance-none flex-1 font-semibold">
          ...
          <div className="relative flex-[2]">
            <input 
              ...
              className="w-full bg-[#0c0c0e] border border-[#e4e4e7]/10 rounded-lg px-4 py-2 text-[#e4e4e7] text-[0.85rem] focus:outline-none font-semibold"
            />
          </div>
          
          <button className="p-2.5 rounded-lg bg-[#27272a] text-[#e4e4e7] hover:bg-[#3f3f46] transition-colors border border-[#e4e4e7]/10 cursor-pointer flex items-center justify-center">
            <RotateCw className="w-4 h-4" />
          </button>
          
          <button onClick={onClose} className="p-2.5 rounded-lg bg-red-600 hover:bg-red-700 text-white transition-colors border border-red-500/30 cursor-pointer flex items-center justify-center">
          ...
          <table className="w-full border-separate border-spacing-y-2">
            ...
            <tbody>
              {filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan={11} className="py-16 text-center text-[#e4e4e7]/60 font-semibold">
                ...
                filteredOrders.map(order => {
                  ...
                  return (
                    <tr key={order.id} className="bg-[#141417] hover:bg-[#1c1c21] hover:scale-[1.002] transition-transform duration-200">
                      ...
                      <td className="py-4 px-4 border-y border-[#e4e4e7]/10">
                        <div className="font-['JetBrains_Mono',monospace] text-[#e4e4e7]/60 text-[0.7rem] mb-0.5">{dt.toISOString().split('T')[0]}</div>
                        <div className="font-['JetBrains_Mono',monospace] font-bold text-[#e4e4e7] text-[0.85rem]">{dt.toTimeString().split(' ')[0].substring(0,5)}</div>
                      </td>
                      ...
                      <td className="py-4 px-4 text-[#e4e4e7]/80 text-[0.8rem] border-y border-[#e4e4e7]/10">
                      ...
                      <td className="py-4 px-4 font-['Inter',sans-serif] text-[#e4e4e7] text-[0.85rem] border-y border-[#e4e4e7]/10">
                        {order.total?.toLocaleString() || '0'}
                      </td>
                      <td className="py-4 px-4 font-bold text-white text-[0.85rem] border-y border-[#e4e4e7]/10">
                      ...
                      <td className="py-4 px-4 text-right border-y border-r border-[#e4e4e7]/10  border-l-0">
                        <button 
                          onClick={() => setSelectedOrder(order)}
                          className="bg-[#10b981] text-white border border-[#10b981] rounded-md p-1.5 cursor-pointer hover:bg-[#059669] transition shadow-[0_0_10px_rgba(16,185,129,0.3)] active:scale-95"
        ...
        {selectedOrder && (
          ...
          >
            <div className="bg-[#141417] border border-white/20 rounded-xl p-6 w-full max-w-[450px] shadow-2xl text-[#e4e4e7] relative font-['Inter',sans-serif] select-text">
              {/* Close Button */}
              <button 
                onClick={() => setSelectedOrder(null)} 
                className="absolute top-3.5 right-3.5 text-stone-400 hover:text-white p-1 rounded-md hover:bg-white/10 transition cursor-pointer"
                title="Close"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="text-center font-bold text-stone-100 text-[0.95rem] pb-2.5">
              ...
              <div className="border-y border-white/25 py-2 flex justify-between items-center text-[0.8rem] font-semibold">
                ...
                <span className="capitalize text-stone-300">{(selectedOrder.orderType || selectedOrder.type || 'delivery').replace('_', ' ')}</span>
              ...
              <div className="py-2.5 space-y-1.5 text-[0.8rem]">
                ...
                <div className="font-bold text-white text-[0.85rem]">
                  {selectedOrder.customer?.name || 'Walk-in'}
                </div>

                <div className="font-['JetBrains_Mono',monospace] text-stone-300">
                  {selectedOrder.customer?.phone || 'N/A'}
                </div>

                <div className="text-stone-300 text-[0.8rem] leading-snug">
              ...
              <div className="border-t border-white/25 py-2.5 space-y-2 text-[0.8rem]">
                {selectedOrder.items && selectedOrder.items.length > 0 ? (
                  selectedOrder.items.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="space-y-0.5">
                        ...
                        {itemNote && (
                          <div className="text-[0.75rem] text-stone-400 pl-1">
                            ({item.name}) {itemNote}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="text-stone-400 text-center py-1">No items recorded</div>
                )}
              </div>
              ...
              <div className="border-t border-white/25 py-2 space-y-1.5 text-[0.8rem]">
                ...
                <div className="border-t border-white/25 pt-2 flex justify-between items-center font-bold text-white text-[0.9rem]">
                  <span>Total</span>
              ...
              <div className="border-t border-white/25 pt-2 text-[0.8rem] space-y-1.5">
                <div className="flex justify-between items-start">
                  <span className="text-stone-400 font-semibold shrink-0">Delivery Note</span>
                ...
                <div className="flex justify-between items-center text-[0.75rem] text-stone-400 pt-1.5 border-t border-white/10">
        ...
        {/* Footer */}
        <footer className="px-8 py-3 border-t border-[#e4e4e7]/10 flex justify-between items-center bg-[#0c0c0e] shrink-0">
          <div className="flex items-center gap-4">
            <div className="font-['JetBrains_Mono',monospace] text-[0.7rem] font-bold bg-[#141417] border border-[#e4e4e7]/10 px-3 py-1.5 rounded-md flex items-center gap-2">
              {filteredOrders.length}
              <select className="bg-transparent text-[#e4e4e7]/60 focus:outline-none appearance-none cursor-pointer">
                <option>15</option>
              </select>
            </div>
            <span className="font-['Inter',sans-serif] text-[0.8rem] text-[#e4e4e7]/60">1-{filteredOrders.length} of {filteredOrders.length} Entries</span>
          </div>
          
          <div className="flex items-center gap-2">
            <button className="bg-[#e4e4e7]/10 text-[#e4e4e7]/60 px-3 py-1.5 rounded-md font-bold text-[0.8rem] cursor-pointer hover:bg-[#e4e4e7]/20 border border-[#e4e4e7]/10">&lt;</button>
            <button className="bg-emerald-500 text-white px-3 py-1.5 rounded-md font-bold text-[0.8rem] cursor-pointer">1</button>
            <button className="bg-[#e4e4e7]/10 text-[#e4e4e7]/60 px-3 py-1.5 rounded-md font-bold text-[0.8rem] cursor-pointer hover:bg-[#e4e4e7]/20 border border-[#e4e4e7]/10">&gt;</button>

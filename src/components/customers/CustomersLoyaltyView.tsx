      ...
      <div className="p-4 bg-white dark:bg-stone-900 border-b border-slate-200 dark:border-stone-800 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="bg-white dark:bg-stone-950 border border-slate-300 dark:border-stone-700 rounded-xl pl-8 pr-3 py-1.5 text-xs text-stone-200 focus:outline-none focus:border-teal-500 w-64 font-medium"
          />
        </div>
      </div>
      ...
      <div className="flex-1 p-5 overflow-y-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((customer) => (
            <div
              key={customer.id}
              className="bg-white dark:bg-stone-900 border border-slate-200 dark:border-stone-800 rounded-2xl p-4 space-y-3 shadow hover:border-teal-500/40 transition"
            >
              ...
              {customer.address && (
                <p className="text-xs text-slate-700 dark:text-stone-300 flex items-start gap-1.5 bg-white dark:bg-stone-950 p-2 rounded-xl border border-slate-200 dark:border-stone-800">
                  <MapPin className="w-3.5 h-3.5 shrink-0 mt-0.5 text-teal-400" />
                  <span className="leading-snug">{customer.address}</span>
                </p>
              )}

              <div className="grid grid-cols-3 gap-2 bg-white dark:bg-stone-950 p-2.5 rounded-xl border border-slate-200 dark:border-stone-800 text-center font-mono">
                <div>
                  <span className="text-[9px] uppercase text-slate-400 dark:text-stone-500 block font-bold">Loyalty</span>
                  <span className="text-xs font-bold text-teal-400">{customer.loyaltyPoints} pts</span>
                </div>
                <div>
                  <span className="text-[9px] uppercase text-slate-400 dark:text-stone-500 block font-bold">Visits</span>
                  <span className="text-xs font-bold text-white">{customer.totalOrdersCount}</span>
                </div>
                <div>
                  <span className="text-[9px] uppercase text-slate-400 dark:text-stone-500 block font-bold">Spend</span>
              ...
              {customer.favoriteItems && customer.favoriteItems.length > 0 && (
                <div className="text-[11px] text-slate-500 dark:text-stone-400">
                  <span className="text-slate-400 dark:text-stone-500 text-[10px] uppercase font-bold block">Favorite Dishes:</span>
                  <span className="text-slate-700 dark:text-stone-300">{customer.favoriteItems.join(', ')}</span>
                </div>
              )}
            </div>
          ))}
